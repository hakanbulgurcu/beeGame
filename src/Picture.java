...
/*    
    public static void main(String[] args) {
        Picture pic = new Picture("Web.png");
...
*/    
     ...
     * left to right.
     */
    public BufferedImage FlipLeftToRight()
    {
     int[][] picture = new int[image.getHeight()][image.getWidth()];
     
        for (int row=0; row<picture.length; row++)
        {
            for (int col=0; col<picture[row].length/2; col++)
            {
            	picture[row][col] = image.getRGB(row, col); 
                int temp = picture[row][col];
                picture[row][col] = picture[row][picture[row].length-1-col];
                picture[row][picture[row].length-1-col] = temp;
            }
        }      
        return image;
    }
    ...
    {
        int[][] picture = new int[image.getHeight()][image.getWidth()];
        int[][] rotatePic = new int[picture[0].length][picture.length];        
        
        for (int row=0; row<rotatePic.length; row++)
        {
            for (int col=0; col<rotatePic[row].length; col++)
            {
            	picture[row][col] = image.getRGB(row, col); 
                rotatePic[row][col]=picture[rotatePic[row].length-1-col][row];
            }
        }      
        return image;              
    }
